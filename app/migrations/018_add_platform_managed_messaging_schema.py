...
def _ensure_column(connection, table_name: str, column_sql: str, logger) -> None:
    ...
    if not columns:
        logger.info("Migration 018: table %s does not exist; skipping column %s.", table_name, column_name)
        return
    if column_name in columns:
        logger.info("Migration 018: %s.%s already present.", table_name, column_name)
        return
    connection.execute(text(f"ALTER TABLE {table_name} ADD COLUMN {column_sql}"))
    logger.info("Migration 018: added %s.%s.", table_name, column_name)
...
def apply(connection, logger) -> None:
    ...
    logger.info("Migration 018: backfilled provider defaults for organization messaging profiles.")

    for statement in TABLE_STATEMENTS:
        connection.execute(text(statement))
    logger.info("Migration 018: ensured provider audit and usage billing tables.")

    for statement in INDEX_STATEMENTS:
        connection.execute(text(statement))
    logger.info("Migration 018: ensured provider audit and usage billing indexes.")
